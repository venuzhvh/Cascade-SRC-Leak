...
implements Util {
    ...
    public static Vec3d getInterpolatedRenderPos(Entity entity, float ticks) {
        return MathUtil.interpolateEntity(entity, ticks).func_178786_a(Minecraft.getMinecraft().func_175598_ae().renderPosX, Minecraft.getMinecraft().func_175598_ae().renderPosY, Minecraft.getMinecraft().func_175598_ae().renderPosZ);
    }
    ...
    public static double[] directionSpeed(double speed) {
        float forward = MathUtil.mc.thePlayer.movementInput.field_192832_b;
        float side = MathUtil.mc.thePlayer.movementInput.moveStrafe;
        float yaw = MathUtil.mc.thePlayer.prevRotationYaw + (MathUtil.mc.thePlayer.rotationYaw - MathUtil.mc.thePlayer.prevRotationYaw) * mc.func_184121_ak();
        if (forward != 0.0f) {
            if (side > 0.0f) {
    ...
    public static RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreNoBox, boolean returnLastUncollidableBlock, boolean ignoreWebs) {
        ...
        if (!(Double.isNaN(vec32.xCoord) || Double.isNaN(vec32.yCoord) || Double.isNaN(vec32.zCoord))) {
            ...
            IBlockState state = MathUtil.mc.theWorld.func_180495_p(pos);
            Block block = state.func_177230_c();
            if (!(ignoreNoBox && state.func_185890_d((IBlockAccess)MathUtil.mc.theWorld, pos) == Block.field_185506_k || !block.func_176209_a(state, stopOnLiquid) || ignoreWebs && block instanceof BlockWeb || (raytraceresult = state.func_185910_a((World)MathUtil.mc.theWorld, pos, vec31, vec32)) == null)) {
            ...
            while (k1-- >= 0) {
                ...
                IBlockState state2 = MathUtil.mc.theWorld.func_180495_p(pos);
                Block block2 = state2.func_177230_c();
                if (ignoreNoBox && state2.func_185904_a() != Material.portal && state2.func_185890_d((IBlockAccess)MathUtil.mc.theWorld, pos) == Block.field_185506_k) continue;
                if (!(!block2.func_176209_a(state2, stopOnLiquid) || ignoreWebs && block2 instanceof BlockWeb)) {
                    RayTraceResult raytraceresult3 = state2.func_185910_a((World)MathUtil.mc.theWorld, pos, vec31, vec32);
    ...
    public static EntityPlayer placeValue(double x, double y, double z, EntityPlayer entity) {
        List list1 = MathUtil.mc.theWorld.func_184144_a((Entity)entity, entity.func_174813_aQ().addCoord(x, y, z));
        if (y != 0.0) {
            int l = list1.size();
            for (int k = 0; k < l; ++k) {
