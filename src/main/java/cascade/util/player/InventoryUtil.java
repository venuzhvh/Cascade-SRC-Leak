...
implements Util {
    ...
    public static void click(int slot) {
        InventoryUtil.mc.playerController.func_187098_a(0, slot, 0, ClickType.PICKUP, (EntityPlayer)InventoryUtil.mc.thePlayer);
    }
    ...
    public static int getStackCount(Item item) {
        ...
        ItemStack offhandStack = InventoryUtil.mc.thePlayer.func_184592_cb();
    ...
    public static boolean heldItem(Item item, Hand hand) {
        switch (hand) {
            case Main: {
                if (InventoryUtil.mc.thePlayer.func_184614_ca().getItem() != item) break;
                return true;
            }
            case Off: {
                if (InventoryUtil.mc.thePlayer.func_184592_cb().getItem() != item) break;
                return true;
            }
            case Both: {
                if (InventoryUtil.mc.thePlayer.func_184592_cb().getItem() != item && InventoryUtil.mc.thePlayer.func_184614_ca().getItem() != item) break;
                return true;
            }
        }
        return false;
    ...
    public static int findHotbarBlock(Class clazz) {
        for (int i = 0; i < 9; ++i) {
            ...
            if (!(stack.getItem() instanceof ItemBlock) || !clazz.isInstance(block = ((ItemBlock)stack.getItem()).func_179223_d())) continue;
            return i;
        }
        return -1;
    }
    ...
    public static int findArmorSlot(EntityEquipmentSlot type, boolean binding) {
        ...
        for (int i = 9; i < 45; ++i) {
            ...
            if (s.getItem() == Items.field_190931_a || !(s.getItem() instanceof ItemArmor) || (armor = (ItemArmor)s.getItem()).func_185083_B_() != type) continue;
            float currentDamage = armor.damageReduceAmount + EnchantmentHelper.getEnchantmentLevel((Enchantment)Enchantments.field_180310_c, (ItemStack)s);
    ...
    public static int findHotbarBlock(Block blockIn) {
        for (int i = 0; i < 9; ++i) {
            Block block;
            ItemStack stack = InventoryUtil.mc.thePlayer.inventory.getStackInSlot(i);
            if (stack == ItemStack.field_190927_a || !(stack.getItem() instanceof ItemBlock) || (block = ((ItemBlock)stack.getItem()).func_179223_d()) != blockIn) continue;
            return i;
        }
        return -1;
    }
    ...
    public static int findInventoryWool(boolean offHand) {
        ...
        for (Map.Entry<Integer, ItemStack> entry : InventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
            ...
            if (wool.func_179223_d().blockMaterial != Material.cloth || entry.getKey() == 45 && !offHand) continue;
            slot.set(entry.getKey());
            return slot.get();
        }
        return slot.get();
    ...
    public static boolean holdingItem(Class clazz) {
        ...
        ItemStack stack = InventoryUtil.mc.thePlayer.func_184614_ca();
        result = InventoryUtil.isInstanceOf(stack, clazz);
        if (!result) {
            ItemStack offhand = InventoryUtil.mc.thePlayer.func_184592_cb();
            result = InventoryUtil.isInstanceOf(stack, clazz);
        }
        return result;
    }
    ...
    public static boolean areStacksCompatible(ItemStack stack1, ItemStack stack2) {
        ...
        if (stack1.getItem() instanceof ItemBlock && stack2.getItem() instanceof ItemBlock) {
            Block block1 = ((ItemBlock)stack1.getItem()).func_179223_d();
            Block block2 = ((ItemBlock)stack2.getItem()).func_179223_d();
    ...
    public static int findArmorSlot(EntityEquipmentSlot type, boolean binding, boolean withXCarry) {
        ...
        if (slot == -1 && withXCarry) {
            ...
            for (int i = 1; i < 5; ++i) {
                ...
                float currentDamage = armor.damageReduceAmount + EnchantmentHelper.getEnchantmentLevel((Enchantment)Enchantments.field_180310_c, (ItemStack)craftingStack);
    ...
    public static class Task {
        ...
        public void run() {
            ...
            if (this.slot != -1) {
                Util.mc.playerController.func_187098_a(Util.mc.thePlayer.inventoryContainer.windowId, this.slot, 0, this.quickClick ? ClickType.QUICK_MOVE : ClickType.PICKUP, (EntityPlayer)Util.mc.thePlayer);
            }
        }
